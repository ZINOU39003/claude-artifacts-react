                ...
                switch (page.type) {
                    ...
                    case 'story_text':
                        ...
                        const imagePositions = [
                            { style: 'position: absolute; top: 40px; right: 40px; transform: rotate(8deg); z-index: 2;' },
                            { style: 'position: absolute; bottom: 40px; left: 40px; transform: rotate(-7deg); z-index: 2;' },
                            { style: 'position: absolute; top: 40px; left: 40px; transform: rotate(-5deg); z-index: 2;' },
                            { style: 'position: absolute; bottom: 40px; right: 40px; transform: rotate(6deg); z-index: 2;' }
                        ];

                        const pos = positions[textPageIndex % positions.length];
                        const imgPos = imagePositions[textPageIndex % imagePositions.length];
                        
                        // FIX: Using layered approach with z-index and strong text-shadow for readability
                        const cartoonBgUrl = bookData[i - 1].imageUrl;
                        const textStyle = `font-family: 'Marhey', sans-serif; font-size: 26px; color: white; line-height: 1.8; font-weight: 700; text-shadow: 0px 2px 6px rgba(0,0,0,0.9);`;
                        ...
                        pageHtml = `<div style="width: ${pdfWidth}px; height: ${pdfHeight}px; position: relative; overflow: hidden;">
                                        ...
                                        <!-- Layer 3: Text -->
                                        <div style="position: absolute; inset: 0; display: flex; padding: 60px; ${pos.container} z-index: 3;">
